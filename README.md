...
- **队伍名单**：真源为 `data/current_roster.json`（静态），后台填选手/队长选人读取之；**选手档案 / 历届名次 / 队色** 亦为静态 JSON，保留不迁库。
...
4. **赛季流程**：
   - **队伍名单**：真源为 `data/current_roster.json`（静态，改名单 = 改 JSON 后重新构建部署；后台填选手/队长选人自动读取，`/admin/teams` 已停用）
   - `/admin/schedule` 建半庄（4 座位选队）→「填选手」（从静态名单下拉）/ 队长 `/captain` 选人（仅本队静态名单）
