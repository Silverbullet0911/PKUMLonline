...
//
// This writes directly into the local Wrangler D1 sqlite file so that
// `npm run pages:dev` immediately shows announcements, schedules, results and
// demo accounts. It does NOT touch remote D1.
//
// Usage:
//   npm run seed:demo
//
...
function pickLocalD1File() {
  if (!existsSync(d1Dir)) throw new Error(`Local D1 directory not found: ${d1Dir}`)
  for (const name of readdirSync(d1Dir)) {
    if (!name.endsWith('.sqlite')) continue
    const file = join(d1Dir, name)
    try {
      ...
      // ignore locked/unreadable files
    }
  }
  throw new Error('No local D1 sqlite with profiles table found. Run `npm run db:migrate:local` first.')
}

const dbPath = pickLocalD1File()
...
async function main() {
  ...
  run(`
    insert or replace into announcements (id, date, title, category, body) values
      ...
      ('00000000-0000-0000-0000-0000000000b2', '2026-09-01', '常规赛赛程说明', '赛程', '本赛季常规赛共 24 个半庄。\n演示环境只预置少量对局，用于查看前台效果。'),
  ...
  for (const g of upcoming) {
    run(`
      insert or replace into games (id, season, stage, date, time, round, status, live_status, seats) values
        ('${g.id}', '26-27', '常规赛', '${g.date}', ${esc(g.time)}, ${esc(g.round)}, 'upcoming', ${esc(g.live_status)}, ${json(g.seats)});
    `)
  }
  ...
  for (const g of finished) {
    run(`
      insert or replace into games (id, season, stage, date, time, round, status, live_status, seats) values
        ('${g.id}', '26-27', '常规赛', '${g.date}', ${esc(g.time)}, ${esc(g.round)}, 'finished', ${esc(g.live_status)}, ${json(g.seats)});
    `)
  }

  db.close()
  console.log(`Seeded local D1: ${dbPath}`)
